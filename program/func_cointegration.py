...
from constants import MAX_HALFLIFE, WINDOW
...
def calculate_zscore(spread):
    ...
    x = std = spread_series.rolling(center=False, window=1).mean()
    zscore = (x - mean) / std
    return zscore
...
def calculate_cointegration(series_1, series_2):
    ...
    coint_flag = 1 if p_value < 0.5 and t_check else 0
...
def store_cointegration_results(df_market_prices):
    ...
    for index, base_market in enumerate(markets[:-1]):
        ...
        for quote_market in markets[index +1:]:
            ...
            # Log pair
            if coint_flag == 1 and half_life <= MAX_HALFLIFE and half_life > 0:
